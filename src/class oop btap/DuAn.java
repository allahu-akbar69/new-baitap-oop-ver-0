...
public class DuAn {
    ...
    private List<NhanVien> DSNhanVien; // Danh sách nhân viên tham gia dự án

    //Constructor không tham số: khởi tạo các thuộc tính bán đầu là rỗng
    public DuAn(){
        this.MaDA = this.TenDA = this.NgayThucHien = null;
    ...
    public boolean checkDate(String n){
        ...
        try {
            ...
            return arr[0].length() == 2 && arr[1].length() == 2 && arr[2].length() == 4 && Integer.parseInt(arr[2]) >= 1980 && Integer.parseInt(arr[2]) <= 2050;
    ...
    public List<NhanVien> searchNhanVienDuAn() {
        DSNhanVien.sort(Comparator.comparing(NhanVien::getTenNV)); // sắp xếp theo thứ tự từ điển của tên nhân viên
        return DSNhanVien;
    }
